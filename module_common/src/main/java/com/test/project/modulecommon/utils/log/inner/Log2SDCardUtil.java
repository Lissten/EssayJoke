 ...
 * Author    zhouchuan
 * Describe：
 * Data:      2019/10/16 16:26
 * Modify by
 * Modification date：
 * Modify content：
 */
public class Log2SDCardUtil {
     ...
     */
    public static void ensureSaveLocationLegal(String saveLocation) {
            ...
            while (true) {
                try {
                    ...
                    if (osw != null) {
                        ...
                        ILogger.e(TAG, "保存log文件的数据输出流未创建成功, 请检查或者关闭BfcLog库的log保存");
                    }
                } catch (InterruptedException e) {
                    ILogger.e(TAG, "保存log到文件,线程读取数据队列出错", e);
                } catch (IOException e) {
                    ILogger.e(TAG, "保存log到文件,写入出错", e);
                }
