...
import com.test.project.modulecommon.utils.log.ILog;
import com.test.project.modulecommon.utils.log.ILogger;
import com.test.project.modulecommon.utils.log.Settings;
...
public class ILogImpl implements ILog {
    ...
    public ILogImpl(Settings settings) {
    ...
    @Override
    public ILog tag(String tag) {
        localTag.set(tag);
        return this;
    }

    @Override
    public ILog thread(boolean showThread) {
        localShowThread.set(showThread);
        return this;
    }

    @Override
    public ILog method(int methodCount) {
    ...
    private void setupStackInfo(Settings settings, StringBuilder builder) {
        ...
        for (int i = MIN_STACK_OFFSET; i < trace.length; i++) {
            ...
            if (ILogImpl.class.getName().equals(name)
                || ILogger.class.getName().equals(name)
                // 对aop插入代码的调用栈进行跳过
